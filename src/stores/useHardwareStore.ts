...
export const useHardwareStore = create<HardwareStoreState & HardwareStoreActions>()(
  persist(
    (set, get) => ({
      ports: [],
      selectedPort: "",
      ...
      scanPorts: async () => {
        ...
        try {
          ...
          set({
            ...
            selectedPort:
              ports.length > 0
                ? get().selectedPort && ports.includes(get().selectedPort)
                  ? get().selectedPort
                  : ports[0]
                : "",
          });
      ...
      connect: async (port?: string) => {
        let targetPort = port || get().selectedPort;
        if (!targetPort) {
          // 若未选择，先扫描一次看看有没有可用端口
          await get().scanPorts();
          targetPort = get().selectedPort;
        }

        if (!targetPort) {
          set({ errorMessage: "未找到任何可用的串口设备" });
          return;
        }
      ...
      initListeners: async () => {
        ...
        try {
          ...
          if (status.connected) {
            ...
            // 未连接：自动扫描并尝试恢复上一次的串口连接
            const savedPort = get().selectedPort;
            get().scanPorts().then(() => {
              const { ports } = get();
              if (savedPort && ports.includes(savedPort)) {
                get().connect(savedPort).catch(() => {});
              }
            });
          }
        } catch {}
